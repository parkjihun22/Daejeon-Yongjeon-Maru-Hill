...
const menuArray = [
  ...
  },
  {
    ...
    subMenu: [
      ...
      { subTitle: "민간임대란?", subUrl: "/SalesInfo/guide" },
      ...
      <>
        ...
        <div className={styles.mobileHeader}>
          <div onClick={() => setIsMobileMenu((v) => !v)}>
            {!isMobileMenu ? (
              <AiOutlineMenu className={styles.icon} size={25} color="#11176b" />
            ) : (
              <IoCloseSharp className={styles.icon} size={25} color="#11176b" />
            )}
          </div>
          {isMobileMenu && (
            <SlideMenu
          ...
          <a href="tel:1533-8848">
            <IoCall className={styles.icon} size={25} color="#11176b" />
          </a>
        </div>
        {isInterestPopupOpen && (
          <InterestPopup
